...
template<typename T, SUBSYSTEM_MODE MODE> std::set<unsigned> subsystem( const std::set<unsigned> &B ) {
    ...
    /**/ if constexpr (MODE == MERGE)   { newset = B; for( auto &id : A ) newset.insert(id); } // then difference,
    else if constexpr (MODE == EXCLUDE) { newset = B; for( auto &id : A ) newset.erase (id); } // then intersection
